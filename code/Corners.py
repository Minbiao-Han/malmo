...
def CornerSearch(maze, start, perm):
    all_paths = []
    seq_path = []
    for i in range(len(perm)):
        ...
        for i in range(len(sequence)):
            if i == 0:
                seq_path = BFS(maze, start, sequence[i])
                #print(start, sequence[i])
            elif i == len(sequence) -1:
                temp = BFS(maze, sequence[i - 1], sequence[i])
                temp.pop(0)
                seq_path += temp
            else:
                temp = BFS(maze, sequence[i - 1], sequence[i])
...
pathC = CornerSearch(level_mat, (pStart['x'], pStart['z']), perm)
